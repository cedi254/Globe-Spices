...
import {chapterState,journeyProgress} from './journey-math.mjs';
...
export default function Journey(){
 ...
 return ( <section className="journey" id="reise" aria-label="Interaktive Gewürzreise"><div className="journey-intro"><p className="eyebrow">DEM GESCHMACK AUF DER SPUR</p><h2>Die Welt dreht sich.<br/><em>Um guten Geschmack.</em></h2><p>Folge deiner Neugier. Scrolle von Land zu Land<br/>oder wähle dein nächstes Reiseziel.</p><a className="journey-skip" href="#kollektion">Reise überspringen <ArrowUpRight size={16}/></a></div><div className="journey-progress" aria-label="Fortschritt der Gewürzreise"><span className="progress-title">DEINE REISE</span><div className="progress-countries">{products.map((p,i)=><button key={p.id} onClick={()=>jump(i)} aria-current={active===i?'step':undefined} style={{'--country':p.color} as React.CSSProperties}>{p.country}</button>)}</div><a className="progress-cta" href="#kollektion">Zur Kollektion <ArrowUpRight size={16}/></a><div className="progress-track" aria-hidden="true"><span style={{width:`${journeyProgress(active,products.length)}%`,background:products[active].color}}/></div></div><div className="journey-overview" aria-label="Die vier Reiseziele">{products.map((p,i)=><button key={p.id} onClick={()=>jump(i)} aria-current={active===i?'step':undefined} style={{'--country':p.color} as React.CSSProperties}><span>0{i+1}</span><strong>{p.country}</strong><small>{p.blend}</small><ArrowUpRight size={16}/></button>)}</div><div className="journey-layout"><div className="globe-stage"><nav className="country-nav" aria-label="Reiseziel wählen">{products.map((p,i)=><button key={p.id} onClick={()=>jump(i)} aria-current={active===i?'step':undefined} style={{'--country':p.color} as React.CSSProperties}><span className="country-number">0{i+1}</span>{p.country}</button>)}</nav><div className="journey-globe" ref={mountRef}>{mounted?<Suspense fallback={<img src="/images/globe-fallback.svg" alt="Weltkarte"/>}><Globe ref={globeRef} onSelect={jump} active={active}/></Suspense>:<img className="journey-placeholder" src="/images/globe-fallback.svg" alt="Weltkarte"/>}</div><div className="globe-caption" aria-live="polite"><MapPin size={16}/><span>{products[active].landmark}</span><span className="caption-count">0{active+1} / 04</span></div><p className="globe-hint">Scrollen & entdecken <ArrowDown size={13}/></p></div>
<div className="chapters">{products.map((p,i)=><article id={p.id} tabIndex={-1} key={p.id} className={`chapter ${active===i?'is-active':''}`} style={{'--country':p.color} as React.CSSProperties}><div className="chapter-heading"><p className="eyebrow"><span className="chapter-index">0{i+1}</span>{p.country.toUpperCase()}</p><h2>{p.title[0]}<br/><em>{p.title[1]}</em></h2></div><div className="chapter-product"><div className="product-photo"><ProductImage product={p}/><span className="weight">{p.weight} · WORLD COLLECTION</span></div><div className="product-copy"><h3>{p.blend}</h3><p className="tasting-note">{p.note}</p><p>{p.description}</p><a className="text-link" href={`/gewuerze/${p.id}`}>Zum Gewürz <ArrowUpRight size={18}/></a></div></div><div className="cooking-note"><Leaf size={18}/><p>{p.use}</p></div></article>)}</div></div></section>
);
}
